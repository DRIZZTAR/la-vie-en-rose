...
camera.position.set(4.5, 4.5, 60);
scene.add(camera);
...
// debugObject.clearColor = '#ff99be';
// renderer.setClearColor(debugObject.clearColor);

/**
 * Load Model
 */
const gltf = await gltfLoader.loadAsync('./model2.glb');
...
particles.material = new THREE.ShaderMaterial({
	...
	uniforms: {
		uSize: new THREE.Uniform(0.666),
		uResolution: new THREE.Uniform(
			new THREE.Vector2(sizes.width * sizes.pixelRatio, sizes.height * sizes.pixelRatio)
		),
		uParticlesTexture: new THREE.Uniform(),
        uDistanceToCenter: new THREE.Uniform(0.5),
	},
});
...
// 	renderer.setClearColor(debugObject.clearColor);
// });
gui.add(particles.material.uniforms.uSize, 'value').min(0).max(10).step(0.001).name('uSize');
gui.add(particles.material.uniforms.uDistanceToCenter, 'value').min(0).max(1).step(0.001).name('uDistanceToCenter');
